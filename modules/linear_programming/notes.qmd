...
Start with the canonical pair above. The dual exchanges the roles of activities and resources:

- Each **primal constraint** becomes a **dual variable**.
- Each **primal variable** becomes a **dual constraint**.
- The primal RHS $b$ becomes the dual objective coefficients.
- The primal objective coefficients $c$ become the dual RHS.
- The coefficient matrix is transposed: $A$ becomes $A^\top$.

**1. A primal constraint determines the sign of its dual variable.**

| Primal constraint | Maximization primal | Minimization primal |
|---|---|---|
| $a_i^\top x\le b_i$ | $y_i\ge0$ | $y_i\le0$ |
| $a_i^\top x\ge b_i$ | $y_i\le0$ | $y_i\ge0$ |
| $a_i^\top x=b_i$ | $y_i$ unrestricted | $y_i$ unrestricted |

To understand the sign, ask: **What happens to the optimal objective value when $b_i$ increases?**

For a $\le$ constraint, increasing $b_i$ relaxes the restriction. Maximum profit cannot fall,
while minimum cost cannot rise. For a $\ge$ constraint, increasing $b_i$ tightens the
restriction, reversing these directions. An equality has no general direction of effect.

**2. A primal variable's sign determines the direction of its dual constraint.**

Let $a_j$ denote column $j$ of $A$, containing the coefficients for primal variable $x_j$.

| Primal variable | Maximization primal → minimization dual | Minimization primal → maximization dual |
|---|---|---|
| $x_j\ge0$ | $a_j^\top y\ge c_j$ | $a_j^\top y\le c_j$ |
| $x_j\le0$ | $a_j^\top y\le c_j$ | $a_j^\top y\ge c_j$ |
| $x_j$ unrestricted | $a_j^\top y=c_j$ | $a_j^\top y=c_j$ |

**In the wheat–corn example:** We maximize profit with three resource-availability
constraints of the form $\le$, so all three dual variables are nonnegative. Both crop
acreages are nonnegative, so both dual constraints take the form $\ge$: the implicit value
of the resources used by each crop must cover its per-acre return.
