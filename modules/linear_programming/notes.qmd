...
- What was the decision the farmer had to make? What did $W$ and $C$ represent?
...
### Algebraic solution: find and compare the vertices

The carbon-abatement model has three decision variables, so a vertex is usually found by setting **three independent constraints** to equality. The candidate binding constraints can be emissions requirements or nonnegativity restrictions such as $B=0$.
...
Doubling the second equation and subtracting the first gives $9M=50$, so $M=50/9$ and $B=5/3$. This point is feasible because its methane reduction is $280/9\approx31.1\geq25$.
...
### One pivot without a full tableau
...
\begin{aligned}
W+C+s_L &= 500,\\
3W+4C+s_{\text{labor}} &= 1800,\\
4W+3C+s_F &= 2000,\\
s_L,s_{\text{labor}},s_F &\geq 0.
\end{aligned}
$$

At the origin, $W=C=0$, so $(s_L,s_{\text{labor}},s_F)=(500,1800,2000)$. This is a BFS: the slack variables form the initial **basis** and record the unused resources.
...
- Let $C=t$ while $W=0$. The slack equations become $s_L=500-t$, $s_{\text{labor}}=1800-4t$, and $s_F=2000-3t$. Feasibility requires
  $$
  t\leq500,\qquad t\leq\frac{1800}{4}=450,\qquad t\leq\frac{2000}{3}\approx666.7.
  $$
- Labor is reached first, so $s_{\text{labor}}$ is the **leaving variable**. This is the **ratio test**: the smallest nonnegative limit determines how far the entering activity can increase.
- The pivot moves us to the neighboring vertex $(W,C)=(0,450)$, where $(s_L,s_{\text{labor}},s_F)=(50,0,650)$ and $z=\$135{,}000$. The basis changes from $\{s_L,s_{\text{labor}},s_F\}$ to $\{s_L,C,s_F\}$.

The algorithm then checks whether bringing wheat into the plan would improve profit. To remain on the binding labor constraint, one additional acre of wheat requires reducing corn by $3/4$ acre. The resulting change in profit is
