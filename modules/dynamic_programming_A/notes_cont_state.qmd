...
## Continuous-Control Version: Calculus Characterization

We now let the extraction choice be continuous: $x_t\in[0,s_t]$. The one-period problem at time $t$ and state $s$ is
$$
\max_{x\in[0,s]}\; g_t(x;s)\equiv p\,x-c(x)+\beta\,V_{t+1}(s-x),
$$
where $V_{t+1}$ is the next-period value function.

### KKT conditions (interior and corners)

Let $\mu,\nu\ge 0$ be the multipliers on $x\ge 0$ and $x\le s$. The KKT conditions are
$$
\begin{aligned}
&\text{(FOC)}\quad p-c'(x)-\beta\,V'_{t+1}(s-x)+\mu-\nu=0,\\
&\text{(Comp. slackness)}\quad \mu\,x=0,\qquad \nu\,(s-x)=0,\\
&\text{(Feasibility)}\quad x\in[0,s],\quad \mu,\nu\ge 0.
\end{aligned}
$$

Hence:

- **Interior optimum** $(0<x<s)$: $\mu=\nu=0$ and
  $$
  \boxed{\,p-c'(x_t^*)-\beta\,V'_{t+1}(s-x_t^*)=0\,}
  \quad\Longleftrightarrow\quad
  c'(x_t^*)=p-\beta\,V'_{t+1}(s-x_t^*).
  $$
- **Corner $x=0$**: $p-c'(0)-\beta\,V'_{t+1}(s)\le 0$.
- **Corner $x=s$**: $p-c'(s)-\beta\,V'_{t+1}(0)\ge 0$.

### Second-order condition (SOC)

$$
g_t''(x;s)=-c''(x)+\beta\,V''_{t+1}(s-x).
$$
A sufficient condition for a maximum is $g_t''(x_t^*;s)<0$. This holds, e.g., when $c''(x)>0$ and $V_{t+1}$ is concave in the stock ($V''_{t+1}\le 0$).

### Envelope (shadow value of the stock)

If an interior optimum applies, the value function’s marginal value of stock satisfies
$$
\boxed{\,V_t'(s)=\beta\,V'_{t+1}\!\big(s-x_t^*(s)\big)\,}
$$
(the derivative of the maximand w.r.t. $s$; the $x$-term drops by the FOC).

---

## Applying to the mine: $p=10,\;c(x)=2x+x^2,\;\beta=0.9$

Here $c'(x)=2+2x,\;c''(x)=2>0$.

### Final decision stage ($t=2$)

Since $V_3\equiv 0$, we solve $\max_{x\le s}\{10x-(2x+x^2)\}$.
The unconstrained optimizer solves $10-c'(x)=0\Rightarrow 10-(2+2x)=0\Rightarrow x=4$.
Thus
$$
x_2^*(s)=\min\{s,4\},\qquad V_2(s)=\pi\big(\min\{s,4\}\big),
$$
and
$$
V_2'(s)=\begin{cases}
...
0,& s>4,
\end{cases}
\quad(\text{kink at }s=4).
$$

### One step back ($t=1$): interior FOC

At state $s$, the interior FOC is
$$
10-(2+2x_1^*)-\beta\,V_2'(s-x_1^*)=0
\quad\Longleftrightarrow\quad
2+2x_1^* = 10-\beta\,V_2'(s-x_1^*).
$$
For any feasible $x_1^*>0$ we have $s-x_1^*<4$, hence $V_2'(s-x_1^*)=8-2(s-x_1^*)$.
Plugging in with $\beta=0.9$:
$$
2+2x_1^* = 10-0.9\,[8-2(s-x_1^*)]
= 2.8+1.8s-1.8x_1^*,
$$
so
$$
3.8\,x_1^* = 0.8+1.8\,s\quad\Rightarrow\quad
\boxed{\,x_1^*(s)=\frac{0.8+1.8\,s}{3.8}\,},
$$
subject to the bounds $x_1^*\in[0,s]$. (If this formula violates a bound, the KKT corner conditions decide $x_1^*=0$ or $x_1^*=s$.)

- Example: at $s=4$, $x_1^*(4)=8/3.8\approx 2.105$ (close to the discrete optimum $2$).
- SOC: $g_1''=-2+0.9\,V_2''(s-x)$; here $V_2''=-2$ for $s-x<4$, so $g_1''=-2-1.8=-3.8<0$.

The envelope then gives $V_1'(s)=\beta\,V_2'(s-x_1^*(s))=0.9\,[8-2(s-x_1^*(s))]$ (for $s-x_1^*(s)<4$).

:::{.callout-note}
Envelope condition

1. Start with the Bellman equation
$$
V_t(s)=\max_{x \in X(s)}  \{ \pi(s,x) + \beta V_{t+1}(f(s,x)) \}.
$$
$V_t(s)$ implicitly defines the maximum value attainable from state $s$ at time $t$. 

2. Differentiate $V_t(s)$ with respect to $s$

Differentiate both sides, applying the chain rule:
$$
\frac{dV_t(s)}{ds}
= \pi_s(s,x_t^*) + \pi_x(s,x_t^*)\,x_t^{*\prime}(s)
+ \beta\,V_{t+1}'(f(s,x_t^*))\,[f_s(s,x_t^*) + f_x(s,x_t^*)\,x_t^{*\prime}(s)].
$$

The derivative contains both:

- **Direct effects** of $s$ on profits and transitions, and  
- **Indirect effects** via how the optimal policy $x_t^*(s)$ changes with $s$.

3. Use the FOC to eliminate $x_t^{*\prime}(s)$

From the inner maximization, the **FOC** is:
$$
\pi_x(s,x_t^*) + \beta\,V_{t+1}'(f(s,x_t^*))\,f_x(s,x_t^*) = 0.
$$

This is exactly the coefficient multiplying $x_t^{*\prime}(s)$ in the derivative above.
Therefore, when the FOC holds, all terms involving $x_t^{*\prime}(s)$ drop out.

4. The envelope condition

After simplification, we obtain the **envelope condition**:
$$
\boxed{
V_t'(s)
= \pi_s(s,x_t^*) + \beta\,V_{t+1}'(f(s,x_t^*))\,f_s(s,x_t^*).
}
$$

This condition tells us how the value function changes with the state $s$, holding the optimal policy fixed.

The envelope theorem says that at the optimum, small changes in the state affect value only through the *direct* channels ($\pi_s$ and $f_s$), not through how the optimal control $x_t^*$ responds to $s$.

- $\pi_s$ captures the **direct marginal benefit** of more state today.  
- The second term captures the **discounted marginal continuation value** of one more unit carried forward.

:::


### Initial decision ($t=0$, $s_0=4$)

Use the interior FOC
$$
10-(2+2x_0^*)-\beta\,V_1'(s_1)=0
\quad\Longleftrightarrow\quad
2+2x_0^* = 10-\beta\,V_1'(s_1).
$$

From the envelope from $t=1$,
$$
V_1'(s_1)=0.9\,[8-2(s_1-x_1^*(s_1))],\quad
x_1^*(s_1)=\frac{0.8+1.8\,s_1}{3.8}, \quad s_1=4-x_0^*
$$
Then substituting $x_1^*(s_1)$ and $s_1$ into $V_1'(s_1)$ and simplifying gives a function of $x_0^*$ alone.
$$
V_1'(s_1)=0.9\left[8-2\left((4 - x_0^*) - \frac{0.8 + 1.8(4 - x_0^*)}{3.8}\right)\right] = \frac{0.9(19.2 + 1.8 x_0^*)}{3.8}.
$$
plugging into the FOC:
$$
2 + 2 x_0^* = 10 - 0.9 \cdot \frac{19.2 + 1.8 x_0^*}{3.8}
\quad\Longleftrightarrow\quad
3.42\,x_0^* = 3.712\quad\Rightarrow\quad
\boxed{\,x_0^* = \frac{3.712}{3.42}\approx 1.085\,}.
$$

you can solve this scalar equation for $x_0^*$ (numerically it lands essentially at $x_0^*\approx 2$, matching the discrete solution). If the interior equation implies $x_0^*\notin[0,4]$, revert to the corner tests:
$$
\begin{aligned}
&x_0^*=0\ \text{if}\ 10-c'(0)-\beta\,V_1'(4)\le 0,\\
&x_0^*=4\ \text{if}\ 10-c'(4)-\beta\,V_1'(0)\ge 0.
\end{aligned}
$$
...
The interior FOC
$$
\boxed{\,c'(x_t^*)=p-\beta\,V'_{t+1}(s_t-x_t^*)\,}
$$
says: *extract until current marginal extraction cost equals the current price minus the discounted marginal value of leaving one more unit in the ground.*  
With convex $c$ and concave $V$, the SOC holds and the optimum is unique.
...
The expectation operator $\mathbb{E}_{p_{t+1} \mid p_t}$ averages future value across all possible next-period prices, weighted by their probabilities. $\mathbb{E}_{p_{t+1} \mid p_t} \big[V_{t+1}(s_t - x_t, p_{t+1})\big]=\sum_{p_t+1} \Pr(p_{t+1}|p_t) V_{t+1}(s_{t+1})$
